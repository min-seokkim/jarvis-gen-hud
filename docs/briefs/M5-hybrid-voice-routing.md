...
> 선행: M0 `voice_smoke_test.py`(로컬 STT→LLM→TTS 경로 스크립트 — **아직 실행·검증 전. S0에서 먼저 돌린다**) · M1 앱 셸 + Hermes `/v1` 스트리밍(완료) · Hermes SOUL 페르소나(완료, `deploy/hermes/`).
...
- **STT:** faster-whisper `small`(또는 `medium`), cuda, float16 — `voice_smoke_test.py`에서 쓴 그대로.
...
- **S0 — 스파이크(선행, 버릴 코드):** `voice_smoke_test.py`를 확장. 같은 입력으로 (a) 로컬 즉답 TTS와 (b) Hermes `/v1` 호출을 asyncio로 **동시에** 띄워, 음성 첫 소리 TTFA와 HUD 첫 토큰 시각을 각각 찍는다. **Exit:** "음성이 클라우드와 무관하게 먼저 난다"를 숫자로 증명.
...
[맥락] AGENTS.md와 docs/기획서.md, 그리고 이 브리프(docs/briefs/M5-hybrid-voice-routing.md)를 먼저 읽어.
       ...
       로컬 경로 스크립트는 voice_smoke_test.py에 있으나 아직 실행·검증 전이다 — S0에서 먼저 돌려라.
[목표] 먼저 S0 스파이크: voice_smoke_test.py를 확장해 "로컬 즉답 TTS"와 "Hermes /v1 호출"을 asyncio로
