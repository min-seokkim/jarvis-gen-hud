...
> **범위 메모 (ADR 0004):** 이번 제출·데모 범위는 **텍스트 명령 → 생성형 HUD**. 음성(STT/TTS)은 future work로 제외. → `docs/decisions/0004-descope-voice.md`
...
- **즉답 + HUD 동시성** — 명령 즉시 자비스가 한 문장으로 응답(usher)하고 화면엔 HUD가 병렬 생성 *(음성 출력은 future work)*
...
| STT / TTS | *future work (범위 제외 — ADR 0004)* · faster-whisper / ElevenLabs |
...
- **2주차~:** generative HUD 코어 루프 → HUD 발명 → HUD 풍부화. *(음성 파이프라인은 future work — ADR 0004)*
...
| 3주차 | (예정) HUD 발명 + HUD 풍부화 + 배포·데모 *(음성은 future work)* |
